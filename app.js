...
function showResults(r){$('resultCard').hidden=false;const fg=$('finalGrade');fg.textContent=r.final;fg.className='result '+(r.final==='HALB'?'grade-bad':(r.final==='KESKMINE'?'grade-warn':'grade-good'));$('scoreLine').textContent=`Arvutuslik hinne: ${r.computed} (${r.overall.toFixed(2).replace('.',',')} p)${r.critical?` · Kriitilisi piiranguid: ${r.critical}`:''}`;$('completeLine').textContent=r.complete?'Staatus: TÄIELIK HINNANG':'Staatus: ESIALGNE – vähemalt ühe osa andmed puuduvad';$('reportMeta').textContent=`Silo: ${currentType} · Prooviveerg: ${activeColumn??'–'} · Analüüsi nr: ${$('analysis').value||'–'} · Ettevõte/hoidla: ${$('farm').value||'–'} · Kuupäev: ${new Date().toLocaleDateString('et-EE')}`;
$('partScores').innerHTML=['A','B','C'].map(p=>r.parts[p]!=null?`<p><strong>${LABELS[p]}:</strong> ${r.parts[p].toFixed(2).replace('.',',')} p – ${grade(r.parts[p])}</p>`:`<p><strong>${LABELS[p]}:</strong> ANDMED PUUDUVAD</p>`).join('');
$('detailBody').innerHTML=r.details.map(x=>`<tr><td>${x.part}</td><td>${x.name}</td><td>${x.v.toFixed(2).replace('.',',')} ${x.unit}</td><td>${x.p}</td><td>${x.g}</td><td class="${x.crit?'crit':''}">${x.crit?'KRIITILINE':''}</td></tr>`).join('');
...
async function ensureTesseract(){if(window.Tesseract)return;setStatus('Laen OCR-moodulit…',5);await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)})}
async function ocrImage(file){
  let worker;
  try{
    await ensureTesseract();
    setStatus('Loen fotolt teksti…',10);
    worker=await Tesseract.createWorker(['eng','deu','est'],Tesseract.OEM.LSTM_ONLY,{logger:m=>{
    ...
    throw new Error('Foto OCR ei käivitunud. Kontrolli internetiühendust. '+e.message);
  }finally{
    if(worker)await worker.terminate().catch(()=>{});
  }
}
async function loadPdfJs(){return await import('https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/+esm')}
...
if('serviceWorker' in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('sw.js').catch(()=>{});renderColumnAssignments();
